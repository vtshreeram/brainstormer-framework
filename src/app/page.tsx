      ...
      <div className="flex justify-between items-start mb-3">
        ...
        >
          <button
            ...
            onClick={(e) => handleAction(e, `/project/${project.id}`)}
          ...
          <button
            title="Edit Wizard"
            onClick={(e) => handleAction(e, `/project/${project.id}/wizard`)}
            className="p-1.5 text-gray-400 hover:text-primary-600 hover:bg-primary-50 rounded transition-colors"
          >
            <svg
              className="w-4 h-4"
          ...
          <button
            title="Open PRD"
            onClick={(e) => handleAction(e, `/project/${project.id}/documents?type=prd`)}
            className="p-1.5 text-gray-400 hover:text-primary-600 hover:bg-primary-50 rounded transition-colors"
          >
            <svg
              className="w-4 h-4"
          ...
          <button
            title="Settings"
            onClick={(e) => handleAction(e, `/project/${project.id}/settings`)}
            className="p-1.5 text-gray-400 hover:text-primary-600 hover:bg-primary-50 rounded transition-colors"
          >
            <svg
              className="w-4 h-4"
        ...
        <Button
          ...
          className={
            isCompleted
              ...
              : "w-full sm:w-auto shadow-sm hover:shadow"
          }
          onClick={(e) =>
            handleAction(
              e,
...
export default function Dashboard() {
  ...
  const filteredAndSortedProjects = useMemo(() => {
    ...
    if (activeFilter === "Recently Edited") {
      ...
      result = result.filter(
        (p) =>
          p.status === "documents_generated" ||
          p.generatedDocuments.length >= 5,
      );
    } else if (activeFilter === "In Progress") {
      result = result.filter(
        (p) =>
          p.status !== "documents_generated" && p.generatedDocuments.length < 5,
      );
    }

    // Apply Default Sorting: Most recently edited first
    result.sort(
      (a, b) =>
        ...
        <div className="mb-8 flex flex-col gap-6">
          <div>
            ...
            <p className="text-gray-500">
              Manage and track your brainstorming sessions and generated
              documents.
            </p>
          </div>
        ...
        {projects.length === 0 ? (
          <div className="bg-white rounded-2xl border border-dashed border-gray-300 p-16 text-center max-w-2xl mx-auto mt-12 shadow-sm">
            ...
            <p className="text-gray-500 mb-8 max-w-md mx-auto text-lg">
              Start your first brainstorming session to generate product
              requirements, user stories, and technical architecture.
            </p>
            <Button
              size="lg"
              onClick={() => setNewProjectModalOpen(true)}
          ...
          <div className="text-center py-20">
            ...
            <p className="text-gray-500">
              We couldn't find anything matching your search and filter
              criteria.
            </p>
            <Button
              variant="ghost"
